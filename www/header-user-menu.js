// header-user-menu.js (MODULE) — stable
...
function buildBell(right) {
  ...
  `;
  btn.addEventListener("click", () => alert("Notifications (bientôt 👀)"));
...
async function applyHeaderUser(user) {
  ...
  if (crownEl) crownEl.style.display = isAdminUser(user) ? "" : "none";
  ...
  if (img && initial) {
    if (url) {
      ...
      initial.textContent = pretty.trim().charAt(0).toUpperCase() || "T";
    }
  }
}
...
onAuthStateChanged(auth, async (user) => {
  if (!user) {
    // IMPORTANT: ne redirige pas depuis login
